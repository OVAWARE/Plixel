...
if __name__=="__main__":
    ...
    parser.add_argument("-noise_mul", help="Noise multiplier (deprecated, use -input_noise_mul and -auto_noise_mul instead). For v1: standard deviation during sampling. For v2: initial noise image influence (0: pure random, 1: only initial image). Default: 6.0 for v1, 0.5 for v2", type=float)
    parser.add_argument("-input_noise_mul", help="Multiplier for input noise image. Higher values increase input image influence. Default: 0.5", type=float, default=0.5)
    parser.add_argument("-auto_noise_mul", help="Multiplier for auto-generated noise. Higher values increase randomness. Default: 6.0 for v1, 0.5 for v2", type=float)
    ...
    # Set default noise multipliers based on version if not provided
    if args.auto_noise_mul is None:
        args.auto_noise_mul = 6.0 if args.noise_version == "v1" else 0.5

    # Use deprecated noise_mul if provided, otherwise use new flags
    input_noise_mul = args.noise_mul if args.noise_mul is not None else args.input_noise_mul
    auto_noise_mul = args.noise_mul if args.noise_mul is not None else args.auto_noise_mul

    if args.noise_version == "v1":
        xs = sample_v1(model, args.num_samples, prompt, display_count=0, noise_mul=auto_noise_mul, 
                    text_scale=args.text_scale, initial_noise=initial_noise)
    else:
        xs = sample_v2(model, args.num_samples, prompt, display_count=0, noise_mul=input_noise_mul, 
                    text_scale=args.text_scale, initial_noise=initial_noise)
