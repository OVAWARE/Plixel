...
def draw_mod(im):
    ...
    if im.shape[2] == 4:
        # If alpha channel is present, convert to RGBA
        return im
    else:
        # If no alpha channel, add one (fully opaque)
        rgba = np.zeros((im.shape[0], im.shape[1], 4), dtype=np.float32)
        rgba[:,:,:3] = im
        rgba[:,:,3] = 1.0
        return rgba
...
def make_spritesheet(ims, fix_width=None, fix_height=None):
    assert fix_width is None or fix_height is None, "Cannot fix both width and height!"
    
    size, size_, channels = ims[0].shape
    assert size == size_, "Spritesheet: input images must be square"
    assert channels == 4, "Expected RGBA images"
    ...
    final = np.zeros((Y*size, X*size, 4), dtype=np.float32)
